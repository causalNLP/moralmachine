...
ggplot() + 
  ...
  geom_point(aes(y=data$gpt4, x=as.numeric(as.factor(data$criterion))),size = 11, shape = 21, fill = "white", stroke = 2, coulor="#0077ad")+
  geom_point(aes(y=data$human, x=as.numeric(as.factor(data$criterion)) ),  size=15, coulor="#fadc02")+
  theme_bw()+
  scale_x_continuous(
    breaks = 1:6,
    labels = c(
...
ggsave("PLOT/fig_tradeoff_swapped.pdf", device=cairo_pdf, width = 27, height = 10)
