## figure with reversed X axis and multiple models pltted as circles
library(ggplot2)
library(dplyr)
library(cowplot)
...
ggplot() + 
  ...
  geom_point(aes(y=data$gpt3.03, x=as.numeric(as.factor(data$criterion)) , colour="#F4ECB7"),  size=15)+
  geom_point(aes(y=data$gpt3.04, x=as.numeric(as.factor(data$criterion)) , colour="#FBDF5B"),  size=15)+
  geom_point(aes(y=data$gpt3.041, x=as.numeric(as.factor(data$criterion)) , colour="#F7CC05"),  size=15)+
  geom_point(aes(y=data$gpt3.042, x=as.numeric(as.factor(data$criterion)) , colour="#DBB402"),  size=15)+
  geom_point(aes(y=data$gpt3, x=as.numeric(as.factor(data$criterion)),col="#B69D04" ),  size=15)+
  geom_point(aes(y=data$gpt3.5, x=as.numeric(as.factor(data$criterion)),col="#877504" ),  size=15)+
  geom_point(aes(y=data$human, x=as.numeric(as.factor(data$criterion)) ,col="#97DA70" ),  size=15)+
  scale_fill_identity() +
  ...
  scale_x_continuous(
    breaks = 1:6, 
    labels = c(
      expression(paste(bold("           Species"),"               Sparing humans  ")),
      expression(paste(bold("No. characters"),"    Sparing more characters  ")),
      expression(paste(bold("                  Age"),"              Sparing the young  ")),
      expression(paste(bold("   Social Status"),"         Sparing higher status  ")),
      expression(paste(bold("             Fitness"),"                  Sparing the fit  ")),
      expression(paste(bold("             Gender"),"                Sparing females  "))
              ),
...
#  annotate("text", x = 6, y = 0.125, label = "LLM's Preference", color = "#0077ad", size = 8)+
  ...
  annotate("text", x = 6, y = 0.15, label = "GPT4's Preference", color = "white", size = 8)+
  scale_y_continuous(breaks=c(0,0.25,0.5,0.75,1),limits=c(0,1),
                   labels=c("100% Favoring the Right", "25-75", "50-50","75-25","100% Favoring the Left"))+
  
  #scale_y_continuous(breaks = c(10,20,30,40, 50,60), labels = c("10","2","3","4","3","4"))+
  theme(
        #legend.position = "right",
        axis.text.y = element_text(hjust=0,color="black",size=35),
...
ggsave("PLOT/fig_tradeoff_swapped_mult.pdf", device=cairo_pdf, width = 27, height = 10)
